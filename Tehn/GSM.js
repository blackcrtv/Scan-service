...
const formatData = (table = [], channels) => {
    ...
    let filteredData = table.map((el, i) => {
        ...
        let mnc = el.system_info.plmn?.mnc?.join('').slice(0, ctry_indicator) ?? el.system_info.mnc?.join('').slice(0, 2);
        let mcc = el.system_info.plmn?.mcc?.join('').slice(0, 3) ?? el.system_info.mcc?.join('').slice(0, 3);

        vecini_filt = vecini_filt.filter(canal => { // lasam doar canalele din fiecare tehnologie
            if (ch_group[`${band}`].includes(canal))
                return canal;
        });
        return {
    ...
    Object.keys(groupData).forEach((item) => {
        let serv = groupData[item].sort((a, b) => {
            if (a.putere > b.putere) return -1;
            if (a.putere < b.putere) return 1;
            return 0;
        }).filter((el) => { if (el.vecini.length) return el })[0];
        servings[item] = { ...serv }
...
const getRecomandare2G = async (data_2G = []) => {
...
const getScore_GSM = (vecini = [], serving = {}) => {
    ...
    let lac_vec = vecini.map((ve) => { return ve.lac });
    let cell_vec = vecini.map((ve) => { return ve.cell_id });

    for (let i = 0; i < unique_arfcn_list.length; i++) { // parcurg fiecare elem din lista de unice
        ...
        for (let j = 0; j < ord_vecini.length; j++) { // parcurg viecare element din vecini
